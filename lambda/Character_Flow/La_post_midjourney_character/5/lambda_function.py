...
def lambda_handler(event, context):
    ...
    try:
        ...
        if style == "2D":
            ...
            prompt = f"<#{object_key}> {my_redirect_url}/{object_key}, in age of {age} years, {gender}, asian, cute, portrait,smiling, full body, main character, emotional, surreal, vibrant, no background, Pixar::2 --iw 0.8 --turbo"
            ...
            prompt = f"<#{object_key}> {my_redirect_url}/{object_key}, in age of {age} years, {gender}, asian, portrait, cute, full body, by Elsa Beskow --turbo"
        else:
            prompt = f"<#{object_key}> {my_redirect_url}/{object_key}, in age of {age} years, {gender}, asian, portrait, cute, full body, by {style} --turbo "
