    ...
    try:
        ...
        if ver != "global":
            ...
            if style == "Anime isekai":
                ...
                prompt = f"<#{object_key}> {cloth_url}::1, {my_redirect_url}/{object_key_divide_version}::2, Portrait of a happy young asian {gender}, {style} --s 50 --no sad, fat, depressed, white background, black background, 3D --turbo "
                if style == "Children's Illustrations":
                    prompt = f"<#{object_key}> {cloth_url}::1, {my_redirect_url}/{object_key_divide_version}::2, Portrait of a happy young asian {gender}, {style}, 2D illustration --s 50 --no sad, fat, depressed, 3D, white background, black background  --turbo "
            ...
            # {my_redirect_url}/{object_key}
            if style == "Anime isekai":
                prompt = f"<#{object_key}> {my_redirect_url}/{object_key_divide_version}, {cloth_url}, {gender}, anime isekai, Portrait, happy, --iw 1.5 --s 50 --turbo"
            elif style == "Pixar::2":
                prompt = f"<#{object_key}> {my_redirect_url}/{object_key_divide_version}, {cloth_url}, {gender}, happy, Portrait, pixar --iw 1.5 --s 50 --turbo"
            elif style == "Studio Ghibli::2, Miyazaki":
                prompt = f"<#{object_key}> {my_redirect_url}/{object_key_divide_version}, {cloth_url}, {gender}, happy, Portrait, Studio Ghibli, Miyazaki --iw 1.5 --s 50 --turbo"
            elif style == "Vincent Van Gogh":
                prompt = f"<#{object_key}> {my_redirect_url}/{object_key_divide_version}, {cloth_url}, {gender}, happy, Portrait, by Elsa Beskow --iw 1.5 --s 50 --turbo"
            elif style == "Ukiyo-e":
                prompt = f"<#{object_key}> {my_redirect_url}/{object_key_divide_version}, {cloth_url}, a portrait of a young {gender}, character, happy, artwork by {style} --s 50 --no sad, fat, tooth --turbo"
            else:
                prompt = f"<#{object_key}> {cloth_url}::1, {my_redirect_url}/{object_key_divide_version}::2, Portrait of a happy young {gender}, {style} --s 50 --no sad, fat, depressed, white background, black background, 3D --turbo "
                if style == "Children's Illustrations":
                    prompt = f"<#{object_key}> {cloth_url}::1, {my_redirect_url}/{object_key_divide_version}::2, Portrait of a happy young {gender}, {style}, 2D illustration --s 50 --no sad, fat, depressed, 3D, white background, black background  --turbo "
            __payload = JsonImagine(MID_JOURNEY_ID, SERVER_ID, CHANNEL_ID, prompt)
