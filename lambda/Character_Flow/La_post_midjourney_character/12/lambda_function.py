...
def lambda_handler(event, context):
    ...
    try:
        ...
        if style == "Anime isekai":
            prompt = f"<#{object_key}> {my_redirect_url}/{object_key_divide_version}, {cloth_url}, photo of a {gender}, asian, anime isekai, smiling, --iw 1.5 --turbo"
        elif style == "Pixar::2":
            prompt = f"<#{object_key}> {my_redirect_url}/{object_key_divide_version}, {cloth_url}, photo of a {gender}, asian, cute, pretty,smiling, pixar --turbo"
        elif style == "Studio Ghibli::2, Miyazaki":
            prompt = f"<#{object_key}> {my_redirect_url}/{object_key_divide_version}, {cloth_url}, photo of a {gender}, asian, cute, pretty, smiling, Studio Ghibli, Miyazaki --turbo"
        elif style == "Vincent Van Gogh":
            prompt = f"<#{object_key}> {my_redirect_url}/{object_key_divide_version}, {cloth_url}, photo of a {gender}, asian, cute, pretty, smiling, by Elsa Beskow --turbo"
        else:
            prompt = f"<#{object_key}> {my_redirect_url}/{object_key_divide_version}, {cloth_url}, photo of a {gender}, asian, cute, pretty, smiling, by {style} --turbo "
