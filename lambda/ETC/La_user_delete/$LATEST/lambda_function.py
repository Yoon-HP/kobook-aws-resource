...
def lambda_handler(event, context):
    ...
    # 이 둘은 따로 처리
    # Dy_midjourney_check_story_prod
    # Dy_midjourney_check_prod

    # Dy_social_book_prod
    try:
        # get image!
        query = f"select * from Dy_social_book_prod where user_id='{user_id}'"
        result = dynamodb_client.execute_statement(Statement=query)

        for item in result["Items"]:
            time_stamp = item["time_stamp"]["S"]
            query = f"delete from Dy_social_book_prod where user_id='{user_id}' and time_stamp='{time_stamp}'"
            temp = dynamodb_client.execute_statement(Statement=query)
    except:
        print("delete fail!!")
        post_slack(f"user delete fail!!! user_id: {user_id}")

    # Dy_story_presigned_url_prod
    try:
        # get image!
        query = f"select * from Dy_story_presigned_url_prod where user_id='{user_id}'"
        result = dynamodb_client.execute_statement(Statement=query)

        for item in result["Items"]:
            time_stamp = item["time_stamp"]["S"]
            query = f"delete from Dy_story_presigned_url_prod where user_id='{user_id}' and time_stamp='{time_stamp}'"
            temp = dynamodb_client.execute_statement(Statement=query)
    except:
        print("delete fail!!")
        post_slack(f"user delete fail!!! user_id: {user_id}")
    ...
    try:
        # get image!
        query = f"select * from Dy_story_quiz_prod where user_id='{user_id}'"
        result = dynamodb_client.execute_statement(Statement=query)

        for item in result["Items"]:
            time_stamp = item["time_stamp"]["S"]
            query = f"delete from Dy_story_quiz_prod where user_id='{user_id}' and time_stamp='{time_stamp}'"
            temp = dynamodb_client.execute_statement(Statement=query)
    except:
        print("delete fail!!")
        post_slack(f"user delete fail!!! user_id: {user_id}")

    # Dy_story_summary_prod
    try:
        # get image!
        query = f"select * from Dy_story_summary_prod where user_id='{user_id}'"
        result = dynamodb_client.execute_statement(Statement=query)

        for item in result["Items"]:
            time_stamp = item["time_stamp"]["S"]
            query = f"delete from Dy_story_summary_prod where user_id='{user_id}' and time_stamp='{time_stamp}'"
            temp = dynamodb_client.execute_statement(Statement=query)
    except:
        print("delete fail!!")
        post_slack(f"user delete fail!!! user_id: {user_id}")
    ...
    try:
        # get image!
        query = f"select * from Dy_story_image_prod where user_id='{user_id}'"
        result = dynamodb_client.execute_statement(Statement=query)

        for item in result["Items"]:
            time_stamp = item["time_stamp"]["S"]
            query = f"delete from Dy_story_image_prod where user_id='{user_id}' and time_stamp='{time_stamp}'"
            temp = dynamodb_client.execute_statement(Statement=query)
    except:
        print("delete fail!!")
        post_slack(f"user delete fail!!! user_id: {user_id}")

    # s3-kkobook-story-image delete
    # s3 delete
    s3 = boto3.resource("s3")
    # s3-kkobook-character
    bucket = s3.Bucket("s3-kkobook-story-image")

    prefix = f"upscale/{user_id}"
