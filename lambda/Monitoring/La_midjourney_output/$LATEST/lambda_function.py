...
def lambda_handler(event, context):
    ...
    try:
        ...
        if "s_t" not in message_body["object_key"]:
            ...
            mode = message_body["object_key"].split("/")[4][:-1]
    ...
    try:
        ...
        if "s_t" not in message_body["object_key"]:
            ...
            if message_body["state"] == "before":
                ...
                # print(message_body)
                # version 2에선 DB에 저장할 때 상태를 after로 바꿈!!! << 백오피스에 안뜨게 하기 위
                ...
                try:
                    ...
                    if flag:
                        ...
                        query = f"select * from Dy_story_event_prod where user_id='{user_id}' and time_stamp='{time_stamp}';"
                        result_mode_check = dynamodb_client.execute_statement(
                            Statement=query
                        )
