    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if job_number >= 4:
                    try:
                        notification = f"from La_post_midjourney_flow_control_story: user_id:{user_id}, time_stamp:{time_stamp}, reason: 즉시 확인 필요!"
    ...
    if mode == 0:
        ...
        for page_index in range(len(keys)):
            ...
            if ver != "global":
                ...
                # global version!
                if object_key:
                    prompt = f"{my_redirect_url}/{object_key}, {gpt_prompt}, 2D illustration, Cinematic Still Shot , Ultra wide angle , wide background,Far Shot ,background, small character, {style} --iw .6 --no realistic, sad, ugly, hands, scream, 3D, white background, black background, s_t/{user_id}/{time_stamp}/{page_index+1}/{mode} --s 50"
                    if style == "watercolor art by Winslow Homer":
                        prompt = f"{my_redirect_url}/{object_key}, {gpt_prompt}, 2D illustration, Cinematic Still Shot , Ultra wide angle , wide background,Far Shot ,background, small character, {style} --iw .6 --no realistic, sad, ugly, hands, scream, 3D, water, white background, black background, s_t/{user_id}/{time_stamp}/{page_index+1}/{mode} --s 50"

                else:
                    prompt = f"{img_url}, {gpt_prompt}, 2D illustration, Cinematic Still Shot , Ultra wide angle , wide background,Far Shot ,background, small character, {style} --iw .6 --no realistic, hands, sad, ugly, scream, books, 3D, white background, black background, s_t/{user_id}/{time_stamp}/{page_index+1}/{mode} --s 50"
                    if style == "watercolor art by Winslow Homer":
                        prompt = f"{my_redirect_url}/{object_key}, {gpt_prompt}, 2D illustration, Cinematic Still Shot , Ultra wide angle , wide background,Far Shot ,background, small character, {style} --iw .6 --no realistic, sad, ugly, hands, scream, 3D, water, white background, black background, s_t/{user_id}/{time_stamp}/{page_index+1}/{mode} --s 50"
        ...
        # 현재 디스코드 채널 상의 job 개수를 고려해 lambda에 전달
        try:
            query = f"SELECT pk from Dy_midjourney_check_story_{env} where \"check\"='yes' or \"check\"='no'"
            result = dynamodb_client.execute_statement(Statement=query)
            # print(result)
        ...
        try:
            ...
            # mode는 이제 필요 없음 (과거 백오피스가 존재하던 시절 사용함)
            query = f"SELECT pk from Dy_midjourney_check_story_{env} where \"check\"='start'"
            result = dynamodb_client.execute_statement(Statement=query)
            print(result)
            # 시간순으로 오래된 요청부터 처리 및 낮은 in_dex부터 생성
            time_sort = []
