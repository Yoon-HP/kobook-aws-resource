...
def lambda_handler(event, context):
    ...
    if mode == 0:
        ...
        for page_index in range(len(keys)):
            ...
            # style 통일!!! (고민) turbo 사용 X
            if object_key:
                prompt = f"<#s_t/{user_id}/{time_stamp}/{page_index+1}/{mode}> {my_redirect_url}/{object_key}, {gpt_prompt}, asian, character, illustration, Far-Shot Angle, {style} --iw .5"
            else:
                prompt = f"<#s_t/{user_id}/{time_stamp}/{page_index+1}/{mode}> {img_url}, {gpt_prompt}, asian, character, illustration, Far-Shot Angle, {style} --iw .5"
            print(prompt)
