...
def lambda_handler(event, context):
    ...
    if mode == 0:
        ...
        for page_index in range(len(keys)):
            ...
            if object_key:
                prompt = f"<#s_t/{user_id}/{time_stamp}/{page_index+1}/{mode}> {my_redirect_url}/{object_key}, {gpt_prompt}, asian, character, 2D illustration, Far-Shot Angle, {style} --iw .6 --no realistic, sad, ugly, hands, scream, 3D, white background"
                if style == "watercolor art by Winslow Homer":
                    prompt = f"<#s_t/{user_id}/{time_stamp}/{page_index+1}/{mode}> {my_redirect_url}/{object_key}, {gpt_prompt}, asian, character, 2D illustration, Far-Shot Angle, {style} --iw .6 --no realistic, sad, ugly, hands, scream, 3D, water, white background"

            else:
                prompt = f"<#s_t/{user_id}/{time_stamp}/{page_index+1}/{mode}> {img_url}, {gpt_prompt}, asian, character, 2D illustration, Far-Shot Angle, {style} --iw .6 --no realistic, hands, sad, ugly, scream, books, 3D, white background"
                if style == "watercolor art by Winslow Homer":
                    prompt = f"<#s_t/{user_id}/{time_stamp}/{page_index+1}/{mode}> {my_redirect_url}/{object_key}, {gpt_prompt}, asian, character, 2D illustration, Far-Shot Angle, {style} --iw .6 --no realistic, sad, ugly, hands, scream, 3D, water, white background"
