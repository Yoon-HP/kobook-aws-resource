...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Dy_story_presigned_url_prod에 put
        temp_dict = {}
        ...
        for page_num in range(len(story_keys)):
            ...
            if page_num == 0:
                ...
                temp.save("/tmp/thumbnail.jpg", quality=95)
                s3.upload_file(
                    "/tmp/thumbnail.jpg",
                    "s3-kkobook-book",
                    f"{user_id}/{time_stamp}/thumbnail.jpg",
                )

                time.sleep(0.5)
                key = f"{user_id}/{time_stamp}/thumbnail.jpg"
                url = generate_presigned_url(
                    s3_client, client_action, {"Bucket": bucket_pre, "Key": key}, 604800
                )
                temp_dict["thumbnail"] = url

            if page_num % 2 == 0:
                # merge
                story = Image.open("/tmp/story.jpg")
                new_image = Image.new(
                ...
                new_image.save("/tmp/page.jpg", quality=95)
            else:
                # merge
                story = Image.open("/tmp/story.jpg")
                new_image = Image.new(
            ...
            s3.upload_file(
                "/tmp/page.jpg",
                "s3-kkobook-book",
                f"{user_id}/{time_stamp}/{page_num+1}.jpg",
            )

            time.sleep(0.5)
            key = f"{user_id}/{time_stamp}/{page_num+1}.jpg"
            url = generate_presigned_url(
                s3_client, client_action, {"Bucket": bucket_pre, "Key": key}, 604800
            )
            temp_dict[str(page_num + 1)] = url
