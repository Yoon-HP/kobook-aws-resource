...
def lambda_handler(event, context):
    ...
    try:
        ...
        # get story!!
        query=f"SELECT * FROM Dy_gpt_story where user_id='{user_id}' and time_stamp='{time_stamp}'"
        print(query)
        ...
        # get image!!
        query=f"SELECT * FROM Dy_midjourney_output_story_upscale where user_id='{user_id}' and time_stamp='{time_stamp}'"
        print(query)
    ...
    try:
        ...
        s3.download_file('s3-kkobook-processing',"font/TmoneyRoundWindExtraBold.ttf",'/tmp/font.tff')
        ...
        for page_num in range(len(story_keys)):
            ...
            write_to_image(text, width, font_path, font_color, result_path)
            ...
            if page_num==0:
                ...
                s3.upload_file('/tmp/thumbnail.png',"s3-kkobook-book",f'{user_id}/{time_stamp}/thumbnail.png')
            # img.save('/tmp/image.png')
            
            # merge
            story=Image.open('/tmp/story.png')
            
            new_image = Image.new('RGB',(2*img.size[0], img.size[1]), (255,255,255))
            ...
            s3.upload_file('/tmp/page.png',"s3-kkobook-book",f'{user_id}/{time_stamp}/{page_num+1}.png')
    ...
    try:    
        ...
        query=f"UPDATE Dy_story_event SET status = 'finish' WHERE user_id='{user_id}' and time_stamp='{time_stamp}';"
        result=dynamodb_client.execute_statement(Statement=query)
        
        
        # title 가져오기
        query=f"select title from Dy_story_event where user_id='{user_id}' and time_stamp='{time_stamp}'"
        result=dynamodb_client.execute_statement(Statement=query)
        ...
        table=dynamodb.Table("Dy_user_book")
        item={}
        # 존재한다면 S3에 책이 만들어진 상태!
