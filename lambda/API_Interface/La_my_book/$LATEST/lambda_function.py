...
def lambda_handler(event, context):
    ...
    # 공유책보기 < 1페이지만 --- 1103: private인 책은 공개되지 않도록 기능 변경
        ...
        if ver!="global":
            try:
                ...
                for item in result["Items"]:
                    ...
                    time_temp.append([time_stamp,user_id,title,num])
                
                time_temp.sort(reverse=True)
                
                for in_dex in range(min(100,len(time_temp))):
                    
                    time_stamp=time_temp[in_dex][0]
                    user_id=time_temp[in_dex][1]
                    title=time_temp[in_dex][2]
                    num=time_temp[in_dex][3]
    ...
    # 메인화면 받아오기 (동화 최대 5권)
    elif httpMethod == "GET" and path.split("/")[-1] == "main":
        # serach dynamodb
        env = "prod"
        # return presigned url!!!
        print("hi main page")
        ...
        if ver!="global":
            try:
                ...
                for item in result["Items"]:
                    ...
                    temp_time_list.append([time_stamp,user_id,title,num])
                
                temp_time_list.sort(reverse=True)
                
                # 최대 5개!
                for tp in range(min(5,len(temp_time_list))):
                    
                    title=temp_time_list[tp][2]
                    num=temp_time_list[tp][3]
                    user_id=temp_time_list[tp][1]
                    time_stamp=temp_time_list[tp][0]
