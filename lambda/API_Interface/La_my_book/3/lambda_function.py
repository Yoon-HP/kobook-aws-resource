    ...
    if httpMethod=="GET":
        ...
        try:
            ...
            for item in result["Items"]:
                ...
                dynamodb_client=boto3.client('dynamodb')
                # print("check!!")
                query=f"select * from Dy_story_presigned_url_prod where user_id='{user_id}' and time_stamp='{time_stamp}'"
                result_presigned_url=dynamodb_client.execute_statement(Statement=query)
                # print(result_presigned_url["Items"][0].keys())
                
                temp["thumbnail"]=result_presigned_url["Items"][0]['thumbnail']['S']
                for i in range(1,9):
                    temp[str(i)]=result_presigned_url["Items"][0][str(i)]['S']
